...
export class TablingShopService {
    ...
    // 오픈|마감
    async openOrClose(data: TablingShopOpenOrCloseType): Promise<ReturnMessageType> {
        const shop = await this.findByPk(data.shop_id);

        // 오픈일 때
        if (data.status === 'OPEN') {
            if (await this.getOpenClose(shop.id)) { // 마감을 하지 않은 매장일 때
        ...
        } else if (data.status === 'CLOSE') { // 마감일 때
